...
class SubscriptionPayService:
    ...
    async def pay_arrear(
        ...
        cache_key = f"idempotency:pay-arrear:{user_id}:{idempotency_key}"
        ...
        ar = await self.db.get(MembershipArrear, arrear_id)
        if not ar or ar.user_id != user_id:
            raise NotFoundError("Задолженность не найдена")
        if ar.status != "open":
            raise AppValidationError("Эта задолженность уже закрыта или недоступна к оплате")
        ...
        description = f"Задолженность членского взноса за {ar.year} г. — Ассоциация трихологов"
