...
from tests.factories import create_doctor_profile, create_plan, create_subscription, create_user
...
async def test_voting_cast_vote_requires_active_subscription(
    client: AsyncClient, db_session: AsyncSession, doctor_user, admin_user
):
    """Doctor without active subscription gets 403 when voting."""
    profile = await create_doctor_profile(db_session, user=doctor_user, status="active")
    # No subscription created — doctor is not an association member
    await db_session.commit()
