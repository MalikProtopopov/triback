...
| `POST` | `/public/events/{id}/register` | Регистрация на мероприятие (с оплатой) |
| `POST` | `/public/events/{id}/confirm-guest` | Подтверждение guest-email |
...
#### POST /api/v1/public/events/{event_id}/register — без изменений формата
...
Страница мероприятия
  ...
  Показать тарифы
       ...
       │
       ▼
  POST /public/events/{id}/register { tariff_id, guest_email?, ... }
       │
       ├── price > 0:
