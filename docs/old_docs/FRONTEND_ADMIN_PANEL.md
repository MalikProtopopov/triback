...
**Дата:** 2026-03-23 (обновлено — голосование: total_eligible_voters с учётом подписки)  
...
**Пояснение:** `total_eligible_voters` — число врачей с одобренным статусом и **действующей подпиской** на момент подсчёта. Рекомендуется отображать подсказку: «Явка: X из Y (Z%). В подсчёт включены только врачи с активной подпиской».
