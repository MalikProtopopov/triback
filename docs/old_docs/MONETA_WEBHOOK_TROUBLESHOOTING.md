...
Moneta вызывает его **до** оплаты. Ответ — XML; бизнес-код в поле `MNT_RESULT_CODE` (HTTP у ответа обычно 200 OK).

**Источник правды по кодам:** официальное описание MONETA.Assistant — [MONETA.Assistant.ru__9_.pdf](new_moneta/MONETA.Assistant.ru__9_.pdf), глава 5 («Проверочные запросы»). Не путать с семантикой полей MerchantAPI.

| `MNT_RESULT_CODE` | Значение (Assistant) | Когда отвечает бэкенд |
|-------------------|----------------------|------------------------|
| **100** | В ответе передаётся сумма заказа | Во входящем Check **не было** параметра `MNT_AMOUNT`; для `pending` возвращаем сумму из БД. |
| **200** | Заказ уже оплачен, уведомление доставлено | Платёж в БД в статусе `succeeded`. |
| **302** | Заказ в обработке | (зарезервировано; сейчас не используется) |
| **402** | Заказ создан и **готов к оплате** | Платёж `pending`, во входящем запросе **уже был** `MNT_AMOUNT` (в ответе всё равно передаём каноническую сумму из БД). |
| **500** | Заказ не актуален в магазине | Нет платежа с таким UUID, статус не `pending`/`succeeded`, или **неверная подпись** входящего Check. |

Примечание из PDF: *«В обычных случаях для продолжения процесса оплаты следует использовать код ответа 402 или 100.»*

Реализация: [`webhooks.py`](../backend/app/api/v1/webhooks.py) (`moneta_check_webhook`). Эндпоинты Moneta **не** ограничиваются rate limiter (запросы идут с серверов Moneta).

---

### Pay URL и Check URL — без throttling

Маршруты `/api/v1/webhooks/moneta`, `/moneta/check`, `/moneta/receipt` намеренно **без** лимита `slowapi`: иначе при общих исходящих IP Moneta возможен `429` и срыв сценария оплаты.
