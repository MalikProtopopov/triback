...
3. **Голосование** — раздел `voting` в `sidebar_sections` доступен врачам. Бэкенд при `POST /voting/{id}/vote` возвращает **403**, если врач не является активным членом (нет подписки или статус не ACTIVE). Обрабатывать 403: показать сообщение «Голосовать могут только активные члены ассоциации» и ссылку на продление подписки. Опционально: скрывать или дизейблить голосование при неактивной подписке (по `GET /subscriptions/status`).
