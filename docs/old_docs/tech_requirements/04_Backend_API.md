...
**Response 403:** врач не является активным членом ассоциации (нет подписки или DoctorProfile.status != ACTIVE)
```json
{ "detail": "Голосовать могут только активные члены ассоциации" }
```
...
2. Проверить что пользователь — активный член ассоциации (DoctorProfile.status = ACTIVE и действующая подписка) — иначе 403
3. Проверить UNIQUE (session_id, user_id)
4. Проверить что candidate_id принадлежит этой сессии
...
**Поле total_eligible_voters:** число врачей со статусом ACTIVE и действующей подпиской (Subscription.status = active, ends_at > now или null).
